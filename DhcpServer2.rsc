...
:foreach i in=[/ip dhcp-server lease find] do={
  :if ([/ip dhcp-server lease get $i status]="bound" && [/ip dhcp-server lease get $i comment]="" && [/ip dhcp-server lease get $i server]="server2") do={
    ...
    # [[:parse [/system script get TG source]] Text=("server2 bound: "."$host - "."$address")];
    /log warning "server2 bound: $host - $address";
  }
} 
