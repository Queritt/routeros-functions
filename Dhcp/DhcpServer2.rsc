...
:foreach n in=[/ip dhcp-server lease find dynamic] do={
    :if ([/ip dhcp-server lease get $n status]="bound" && [/ip dhcp-server lease get $n comment]="" && [/ip dhcp-server lease get $n server]="server2") do={
        :local address [/ip dhcp-server lease get $n active-address];
        :local host [/ip dhcp-server lease get $n host-name];
        /ip dhcp-server lease set $n comment=("Guest"."-".$sysDate."-".$sysTime);
        /log warning "Dhcp server2 lease: $address $host";
    }
}
